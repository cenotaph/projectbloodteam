...
class IMDB
    attr_accessor :movie_name
  	class << self
  		...
  		def title_search(title)
  			...
  			z = []
  			doc.search("p").each{|x| z.push(x) if  !x.search("table").nil? && !x.search("b").first.nil? &&  x.search("b").first.inner_html.to_s.downcase.include?("title") }
  			p = {}
  			z.each{|x| p.merge!({x.search("b").first.inner_html => x.search("a").collect{|z| z.inner_html}.delete_if{|u| u.include?("<img")}.uniq})}
  			return p
  		end
  	end
      ...
      @page_html = nil
      @movie_name = @movie_name.to_s.gsub(/\/combined$/, '')
      @html_info_tags = nil
      @imdb_link = nil
  		if (doc/"td#img_primary/.image/a/img").first.nil?
  		  ""
  		else
  		  (doc/"td#img_primary/.image/a/img").first["src"]
  		end
  	end
  	...
  	def poster_small
  		...
  		if (doc/"//a[@name='poster']/img").first.nil?
  		  ""
  		else
  	...
  	def tagline
  		...
  		if elements.first
        elements.first.search("div").text.strip.gsub(/\sSee more.+$/, '')
  		else
  			""
  		end
  	end
  	...
  	def release_date
  		...
  		if z.empty?
  		  ...
  		  return z.first.search("div").text.strip.gsub(/\s+See more.+$/, '')
  		end
  	end
    ...
    def year
        doc = load_page
        y = doc.search("h1").first.inner_html.split(%r(<span>))[1]
        return clean_html_tags(y).scan(/\((\d\d\d\d)\/*\w*\)/)[0][0]
    end
  	...
  	def plot
  		...
  		if elements.first
  			elements.first.search("div").text.strip.gsub(/\s+See more.+$/, '').gsub(/\s+Full summary.+$/, '').gsub(/\s+\|\s(Add|Full)\ssynopsis(.+)$/, '')
  		else
  			""
  		end
  	end
  	...
  	def runtime
  		...
  		if elements.first
  			elements.first.search("div").text.strip
  		else
  			""
  		end
  	end
  	...
  	if elements.first
    	  return elements.first.search("a").append(" / ").inner_html.gsub(/\s\/\s$/, '')
  		else
  			""
  		end
  	end
    ...
    #@html_info_tags = doc.search("div[@class='info']")
  	def pbt_menu
        ...
        movie_name.gsub!(/1$/, '')
        if movie_name =~ /^tt(\d+)$/
          connection = open(URI.encode("http://www.imdb.com/title/tt" + movie_name.match(/^tt(\d+)$/)[1] ))
        else
          connection = open(URI.encode("http://www.imdb.com/find?s=all&q=#{movie_name}"))
  			...
  			#find all links
			  array = []
      	unless doc.search("h1").first.nil?   # we have one exact hit
				  if (doc/"/html/head/title").inner_html!="IMDb Search"
    				key = doc.search("link").delete_if{|x| x.attributes['rel'] != 'canonical' }.first.attributes['href'].scan(/title\/tt(\d{5,})/)
            if key[0].nil?
              
            else
    				  title = (doc/"/html/head/title").inner_html.gsub(/\s\-\sIMDb$/, '')
  		        hit = {'key' => key[0][0], 'title' => title.gsub(/\)$/, '') }
  		        array.push(hit)
            end
          end
        end
        ...
        count = 0
  		  elements.each{|link|
  			...
  			if link.attributes['href'] && link.attributes['href'].include?('/title/') && count<@MAXCOUNT
  			  next unless link.next_node.to_s =~ /^\s*\(\d{4}\/?\w?\)\s*$/
  				index = link.attributes['href'].gsub(/\D/, '').gsub(/1$/, '').to_i
  				year = link.next_node.to_s.strip
  				next unless year =~ /^\(\d\d\d\d\/?\w?\)/
  				hit=  {'key' => index, 'title' => link.inner_html + " " + year.gsub(/\,\s\.+\)\)/, ')')}
  				unless array.include?(hit)
  				  array.push(hit)
  				end
  			end
  		}
  	...
  	def load_page(full = nil)
  		...
  		if @page_html
  			...
  			if movie_name =~ /^\d{4,}/
  			  if full.nil?
            begin
  			     connection = open("http://www.imdb.com/title/tt#{movie_name}/combined") 
           rescue
              abort "http://www.imdb.com/title/tt#{movie_name}/combined"
            end
  			  else
  			    connection = open("http://www.imdb.com/title/tt#{movie_name}") 
  			  end
  			else
  			  connection = open(URI.encode("http://www.imdb.com/find?s=all&q=#{movie_name}"))
  			end
  			...
  			begin
  			elements.each{|link|
  				if link.parent.inner_html =~ /<a\shref=["|']\/title\/tt\d*\/["|'].*>.*?<.a>\s\(\d\d\d\d\)/ 
  			...
  			if doc.search("h1").first.nil?
  				...
  				if !@interactive_load
  					...
  					@imdb_link = "http://www.imdb.com"+pbt_menu(elements)
  				end
  ...
  if $0==__FILE__
  	begin
  		["scotland pa","MaLlRaTs"].each{|movie|#,"Doctor+Zhivago","blue_velvet","die hard","die hard 2","ghost in the shell","pi","office_space","11:14","high plains drifter"].each{|movie|
  			next unless movie
  			puts movie
