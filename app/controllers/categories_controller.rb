...
class CategoriesController < ApplicationController
  ...
  def update
    ...
    if @category.agent != current_agent
     ...
     params[:category].each do |p|
       ...
       if p =~ /^has/
         if params[:category][p] == '0'
           @category.update_column(p.sub(/^has_/,'').to_sym, 0)
            # params['category'][p[0].sub(/^has_/,'')] = '0'
         else
           @category.update_column(p.sub(/^has_/,'').to_sym, params[:category][p.sub(/^has_/, '')])
  ...
  def category_params
    params.require(:category).permit(:movies, :musics, :books, :bars, :restaurants, :concerts, :events, :eating, :exercise, :weight, 
    :musicplayed, :activities, :airports, :videogames, :groceries,:brewing, :takeaway, :gambling, :tvseries, :year, :miles,
     :has_movies, :has_musics, :has_books, :has_activities, :has_airports, :has_brewing, :has_concerts, :has_eating, :has_events, 
     :has_exercise, :has_gambling, :has_groceries, :has_miles, :has_musicplayed, :has_restaurants, :has_takeaway, :has_bars, 
     :has_weight, :has_videogames, :has_tvseries)
  end
