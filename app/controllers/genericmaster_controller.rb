...
class GenericmasterController < ApplicationController
  ...
  before_action :authenticate_agent!, :only => [:new, :edit, :edit_master, :query, :update, :create, :destroy, :directid, :authenticate, :callback, :unreviewed]
  theme :getTheme
  ...
  def undated
    require 'discogs'
    offset = rand(@category.classify.constantize.where("comment NOT LIKE  '%]'").where(:agent_id => current_agent.id).count)
    @item = @category.classify.constantize.where("comment NOT LIKE '%]'").where(:agent_id => current_agent.id).offset(offset).first
  ...
  def permitted_params
    params.permit(:master_music  => [:artist, :title, :label, :year, :format, :discogscode], :master_book => [:title, :author, :open_library, :amazoncode, :filename], :master_movie => [:title, :director, :year, :country, :imdbcode, :filename, :english_title])
  end
