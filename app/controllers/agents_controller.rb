...
class AgentsController < ApplicationController
  ...
  def show
    ...
    if @profile.nil?
      ...
      #@json = Entry.where(:agent => @agent).order('created_at DESC').limit(25).to_a.delete_if{|x| !x.entry.respond_to?('geolocation_id')}.delete_if{|x| x.entry.geolocation_id.nil? }.map{|x| x.entry.geolocation }.compact
