...
class Comment < ActiveRecord::Base
 ...
 attr_accessor :currency_id
  ...
  def self.chatter_since_last(last_login)
    ...
    sql = "select cc.foreign_id, cc.item_type, max(cc.updated_at) from (select c.foreign_id, c.item_type, c.updated_at from (  select id, foreign_id, agent_id, max(updated_at) as updated_at, item_type from comments WHERE updated_at > '#{last_login}' GROUP BY foreign_id, item_type , id UNION select id, id as foreign_id, agent_id, updated_at, 'Forum' as item_type from forums WHERE updated_at > '#{last_login}' GROUP BY foreign_id, item_type, id  ) c group by c.foreign_id, c.item_type, c.updated_at  ORDER by c.updated_at DESC ) cc group by cc.foreign_id, cc.item_type order by cc.updated_at desc "
