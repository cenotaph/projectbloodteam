...
xml.rss :version => "2.0" do
  xml.channel do
    ...
    for article in @newsfeed.map(&:entry) do
      ...
      xml.item do
        if article.respond_to?('metadata') 
          ...
          mydesc += "<br />"
          mydesc += article.metadata['comment'].nil? ? "<em>This agent has not yet commented.</em>" : article.metadata['comment']
        end
          xml.title  article.class.to_s + ": " + article.name.gsub(/<\/?[^>]*>/,  "").gsub(/\s+/, ' ').strip
